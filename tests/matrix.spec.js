...
test("matrix filters, detail, exports and keyboard", async ({ page }) => {
  ...
  await expect(page.locator(".cell-button")).toHaveCount(0);
  await expect(page.locator(".empty-cell")).toHaveCount(24);
  await page.getByLabel("仅异常").check();
  await expect(page.locator(".cell-button")).toHaveCount(0);
  await page.getByLabel("仅异常").uncheck();
  await page.getByLabel("渠道", { exact: true }).selectOption("retail");
  ...
  await page.getByLabel("容量", { exact: true }).selectOption("256GB");
  await page.getByLabel("渠道", { exact: true }).selectOption("retail");
  const trigger = page.getByRole("button", { name: "沙特阿拉伯 Galaxy A57 价格详情" });
  await expect(trigger).toHaveCount(1);
  await trigger.click();
  const dialog = page.getByRole("dialog");
  await expect(dialog).toBeVisible();
  await expect(dialog.locator(".record")).toHaveCount(1);
